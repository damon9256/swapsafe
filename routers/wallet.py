...
def get_balances(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "wallets": [{"network": w.network, "address": w.address, "available": round(w.balance, 6), "locked": round(w.locked, 6)} for w in wallets]
    }
...
def get_deposit_addresses(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    wallets = db.query(models.Wallet).filter(models.Wallet.user_id == current_user.id).all()
    return {w.network: {"address": w.address, "confirmations_req": settings.CONFIRMATIONS.get(w.network.value, 12) if hasattr(settings, 'CONFIRMATIONS') else 12} for w in wallets}
...
def withdraw(req: WithdrawRequest, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    withdrawal = models.Withdrawal(user_id=current_user.id, network=req.network, to_address=req.to_address, amount=req.amount, gas_fee=gas_fee, platform_fee=0.0, status="PENDING")
    db.add(withdrawal)
    db.commit()
    db.refresh(withdrawal)
    send_notification(current_user.telegram_id, f"⏳ Withdrawal of {req.amount} USDT ({req.network.value}) submitted.")
    return {"id": withdrawal.id, "status": "PENDING", "amount": req.amount, "gas_fee": gas_fee, "message": "Withdrawal submitted."}
...
def get_deposit_history(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    deposits = db.query(models.Deposit).filter(models.Deposit.wallet_id.in_(wallet_ids)).order_by(models.Deposit.created_at.desc()).limit(50).all()
    return [{"txid": d.txid, "amount": d.amount, "network": d.network, "confirmations": d.confirmations, "credited": d.credited, "created_at": str(d.created_at)} for d in deposits]

@router.get("/withdrawals")
def get_withdrawal_history(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    withdrawals = db.query(models.Withdrawal).filter(models.Withdrawal.user_id == current_user.id).order_by(models.Withdrawal.created_at.desc()).limit(50).all()
    return [{"id": w.id, "amount": w.amount, "gas_fee": w.gas_fee, "network": w.network, "to_address": w.to_address, "status": w.status, "txid": w.txid, "created_at": str(w.created_at)} for w in withdrawals]

def _estimate_gas_fee(network: models.Network) -> float:
    fees = {models.Network.TRC20: 1.0, models.Network.BEP20: 0.5, models.Network.POLYGON: 0.1, models.Network.SOLANA: 0.01, models.Network.ARBITRUM: 0.5, models.Network.OPTIMISM: 0.5, models.Network.ERC20: 5.0}
